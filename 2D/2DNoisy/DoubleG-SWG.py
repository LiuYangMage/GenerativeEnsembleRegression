...
n_W = [tf.Variable(np.zeros((1,)), dtype = dtype)]
...
if args.GAN == 'SW':
    ...
    for i in range(frames):
        ...
        thisG = Gs[steps[i]] + tf.random.normal([bs, zdim], dtype = dtype) * tf.nn.softplus(n_W[0])
        ...
        projected_fake = tf.transpose(tf.matmul(thisG, theta))
    ...
    G_op = tf.train.AdamOptimizer(learning_rate = args.lr).minimize(loss_PQ_all, var_list = G_W + G_b + d_W + d_b + s_W + n_W)
    ...
    for i in range(frames):
        ...
        thisG = Gs[steps[i]] + tf.random.normal([bs, zdim], dtype = dtype) * tf.nn.softplus(n_W[0])
        surrogate_loss = tf.reduce_mean(d(Qs[i]) - d(thisG))
        loss_PQ[i] = surrogate_loss
        loss_D[i] = - surrogate_loss + lamda *  gradient_pernalty(thisG, Qs[i], d, batch_size = bs)

        print(i, end = ' ', flush = True)

    loss_PQ_all = tf.reduce_sum(loss_PQ)  
    G_op = tf.train.AdamOptimizer(learning_rate = args.lr, beta1=0.5, beta2=0.9).minimize(loss_PQ_all, var_list = G_W + G_b + d_W + d_b + s_W + n_W)
...
if args.restore >= 0:
    ...
    diff_history = [np.array(A) for A in np.load(savedir+'/noise_history.npz')['noise_history']][:-1]
...
for _ in range(args.iterations - it + 1):
    ...
    if it % 500 ==0:
        ...
        noise_history.append(sess.run(n_W))
        np.savez(savedir+'/noise_history.npz', noise_history = np.array(noise_history))
        print(noise_history[-1])
